    ...
    implements TableModel
{
    ...
    protected Set    m_urlsAll        = new HashSet();
    protected Vector m_statsAll       = new Vector();
    protected Set    m_statsToStart   = new HashSet();
    protected Set    m_statsDone      = new HashSet();
    protected Set    m_statsRunning   = new HashSet();
    protected Set    m_threadsRunning = new HashSet();
    ...
    protected Vector m_threadListeners = new Vector();
	...
	if (m_statsAll != null) {
		...
		Comparator c = ((PageStats)(m_statsAll.get(0))).getComparator(columnIndex, direction);
	...
	return ((PageStats)m_statsAll.get(row)).getRunState();
    }

    public Color getRowStateColor(int row) {
	return ((PageStats)m_statsAll.get(row)).getRunStateColor();
    }

    public void startNextThread() {
	PageStats stats = (PageStats) m_statsToStart.iterator().next();
...
//	  } else {
	...
	Vector listeners = new Vector(m_tableModelListeners);
    ...
    /* The tableModel internface. This uses the status of all the threads as the table data. */
    protected Vector m_tableModelListeners = new Vector();
    ...
    public synchronized Object getValueAt(int rowIndex,
					  int columnIndex) {
	return ((PageStats)m_statsAll.get(rowIndex)).getColumn(columnIndex);
    }
	...
	JenuThreadEvent e = new JenuThreadEvent
	    (
	     this,
	     m_statsAll.size(),
	     m_statsDone.size(),
	     m_concurrentThreads,
	     m_threadsRunning.size(),
	     m_statsToStart.size()
	     );
...
//	  public TmURL(String protocol, String host, int port, String file) { super(protocol, host, port, file); }
//	  public TmURL(String protocol, String host, int port, String file, URLStreamHandler handler) { super (protocol, host, port, file, handler); }
//	  public TmURL(String protocol, String host, String file) { super(protocol, host, file); }
//	  public TmURL(URL context, String spec) { super(context, spec); }
//	  public TmURL(URL context, String spec, URLStreamHandler handler) { super (context, spec, handler); }
//	  
